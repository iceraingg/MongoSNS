...
def getPosts(db,userid):
    print("=============<{:^21s}>=============".format("MY WALL"))
    posts = db.post.find({"user_id":userid}).sort([("post_time",-1)])
    post_ids = []
#         print("dkdkdk")
    while True:
        for i in range(5):
...
{:^49}
{:^49}
                    """.format("<CHOOSE YOUR MENU>","1. See comment","2. Write comment","3. Add like","4. Cancel like","Exit"))
                    print('-'*49)
